...
const EmployeeCard = ({ employee, onEmployeeClick }) => {
  ...
  return (
    <div 
      onClick={handleClick}
      className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-lg transition-all duration-200 cursor-pointer hover:border-blue-300 group"
    >
      {/* Employee Photo and Name */}
      <div className="flex flex-col items-center text-center">
        {/* Profile Image */}
        <div className="w-16 h-16 rounded-full overflow-hidden mb-3 group-hover:scale-105 transition-transform duration-200">
          <img 
            src={employee.image} 
            alt={employee.name}
        ...
        <p className="text-sm text-gray-600 capitalize font-medium mb-3">
