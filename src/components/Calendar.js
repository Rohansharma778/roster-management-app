...
const Calendar = ({ selectedEmployee, onDateSelect, isManagementView = false, employees = [] }) => {
  ...
  // Get button style for slots - Following exact requirements:
  // Green: Online | Orange: Offline | Blue: Online + Offline | Brown: Blocked | Gray: Available Slots
  const getSlotButtonStyle = (slot) => {
    switch (slot.status) {
      case 'online':
        return 'bg-green-500 text-white hover:bg-green-600 cursor-pointer'; // Green: Online
      case 'offline':
        return 'bg-orange-500 text-white hover:bg-orange-600 cursor-pointer'; // Orange: Offline
      case 'online-offline':
        return 'bg-blue-500 text-white hover:bg-blue-600 cursor-pointer'; // Blue: Online + Offline
      case 'online-booked':
        return 'bg-green-800 text-white cursor-not-allowed'; // Dark Green: Online Booked
      case 'offline-booked':
        return 'bg-orange-800 text-white cursor-not-allowed'; // Dark Orange: Offline Booked
      case 'blocked':
        return 'bg-yellow-800 text-white cursor-not-allowed'; // Brown: Blocked (using yellow-800 for brown)
      case 'unavailable':
      default:
        return 'bg-gray-400 text-gray-700 cursor-not-allowed'; // Gray: Available Slots
    }
  };
    ...
    <div className={darkMode ? 'bg-gray-900' : 'bg-white'}>
      ...
      <div className={`border-b p-4 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className={`text-base sm:text-lg font-medium mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto justify-end">
            ...
            <div className={`p-1 rounded-full flex shadow-sm ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
              <button
                onClick={() => setViewMode('slots')}
                className={`flex items-center justify-center w-12 sm:w-14 h-9 sm:h-10 rounded-full transition-all duration-200 touch-manipulation ${
                  viewMode === 'slots'
                    ? darkMode ? 'bg-gray-600 shadow-sm text-white' : 'bg-white shadow-sm text-gray-800'
                    : darkMode ? 'text-gray-300 hover:text-white' : 'text-gray-500 hover:text-gray-700'
                }`}
                title="Slots View"
                aria-label="Switch to slots view"
              >
                <svg className="w-4 sm:w-5 h-4 sm:h-5" fill="none" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <button
              onClick={toggleDarkMode}
              className={`flex items-center justify-center w-10 sm:w-12 h-9 sm:h-10 rounded-lg transition-all duration-200 border touch-manipulation ${
                darkMode 
                  ? 'bg-gray-700 border-gray-600 text-yellow-400 hover:bg-gray-600' 
                  : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50'
              }`}
      ...
      <div className={`p-4 sm:p-6 ${darkMode ? 'bg-gray-900' : ''}`}>
        {/* Date Navigation Bar */}
        <div className={`mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
          <div className="flex items-center justify-center gap-3 sm:gap-4">
            <button
              onClick={handlePrevWeek}
              className={`p-2 sm:p-2 rounded-lg transition-colors touch-manipulation ${
                darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-200'
              }`}
              aria-label="Previous week"
            >
              <svg className={`w-4 sm:w-5 h-4 sm:h-5 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div className="flex gap-1 sm:gap-2 overflow-x-auto">
              {weekDates.map((dateInfo) => (
                <button
                  ...
                  className={`px-3 sm:px-4 py-2 rounded-lg text-center transition-all duration-200 min-w-[70px] sm:min-w-[80px] touch-manipulation ${
                  ...
                  aria-label={`Select ${dateInfo.day} ${dateInfo.date}`}
                >
                  <div className="text-xs font-medium opacity-75">{dateInfo.day}</div>
                  <div className="text-base sm:text-lg font-bold mt-1">{dateInfo.date}</div>
                </button>
              ))}
            </div>
        ...
        {/* INFO Section - Color Legend per Requirements */}
          ...
          <div className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">
                ...
                <span className={darkMode ? 'text-blue-300' : 'text-gray-600'}>Green: Online</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 bg-orange-500 rounded"></div>
                <span className={darkMode ? 'text-blue-300' : 'text-gray-600'}>Orange: Offline</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 bg-blue-500 rounded"></div>
                <span className={darkMode ? 'text-blue-300' : 'text-gray-600'}>Blue: Online + Offline</span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 bg-yellow-800 rounded"></div>
                <span className={darkMode ? 'text-blue-300' : 'text-gray-600'}>Brown: Blocked</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 bg-gray-400 rounded"></div>
                <span className={darkMode ? 'text-blue-300' : 'text-gray-600'}>Gray: Available Slots</span>
              </div>
            </div>
          </div>
        </div>
